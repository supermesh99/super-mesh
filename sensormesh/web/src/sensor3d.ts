/**
 * Procedural 3D assets for SensorMesh — a physically-modelled sensor device
...
export type ModelKind = "sensor" | "chip" | "gyro";
...
export function mountModelViewer(
  ...
  const model = kind === "chip" ? buildSecureChip() : kind === "gyro" ? buildGyroCore() : buildSensorDevice();
  scene.add(model);
